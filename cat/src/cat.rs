...
fn print_number(content: String, writer: &mut impl Write) {
    ...
    for (i, line) in content.lines().enumerate() {
        writeln!(writer, "{:>width$\t} {line}", i + 1, width = digits_of_nof_lines)
...
mod tests {
    ...
    fn one_line_replace() {
        let content = String::from("Hello World!");
        let options = Options { show_ends: true, ..Default::default() };
    ...
    fn one_line_number() {
        let content = String::from("Hello World!");
        let options = Options { number: true, ..Default::default() };
